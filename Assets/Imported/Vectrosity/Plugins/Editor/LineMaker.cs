...
public class LineMaker : ScriptableWizard {
	...
	private void OnGUI () {
		...
		GUILayout.BeginHorizontal();
		if (GUILayout.Button ("Generate Line", GUILayout.Width(buttonWidth))) {
			message = "";
			ComputeLine();
		}
		if (GUILayout.Button ("Write Line to File", GUILayout.Width(buttonWidth))) {
			WriteLineToFile();
		}
	}
	...
	private void ComputeLine () {
		...
		for (int i = 0; i < linePoints.Count; i++) {
			...
			if (!useVector2) {
				output += "new Vector3(" + p1.x + "f" + ", " + p1.y + "f" + ", " + p1.z + "f" + "), "
						+ "new Vector3(" + p2.x + "f" + ", " + p2.y + "f" + ", " + p2.z + "f" + ")";
			}
			else {
				output += "new Vector2(" + p1.x + "f" + ", " + p1.y + "f" + "), "
						+ "new Vector2(" + p2.x + "f" + ", " + p2.y + "f" + ")";
			}
