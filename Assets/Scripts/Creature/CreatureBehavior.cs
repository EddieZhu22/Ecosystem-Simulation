...
public class CreatureBehavior : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        brain.food = 10;
        brain.water = Mathf.Infinity; // Not Implemented
        brain.action = 0;
        SetDetails();
    ...
    private void Update()
    {
        ...
        brain.Movement();
        if (brain.food >= -(stats.genes[9]*brain.maxStorage) + brain.maxStorage && brain.age > 10 && brain.mated == false)
        {
            toggle = true;
        }
        if (toggle == false)
        ...
        if (brain.food < (-(stats.genes[9] * brain.maxStorage) + brain.maxStorage)/3 && toggle == true)
        {
            toggle = false;
        }
        if (toggle == true)
        {
            if (brain.mated == false)
                brain.FindMate();
        }
        if (brain.mated == true)
        {
            ...
            if (brain.refractoryPeriod > refractoryTime)
            {
                brain.foundMate = false;
                brain.mated = false;
            }
        }
        if (year < 0)
